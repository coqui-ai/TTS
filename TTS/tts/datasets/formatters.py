...
def coqui(root_path, meta_file, ignored_speakers=None):
    ...
    speaker = None if "speaker_name" in metadata.columns else "coqui"
    emotion = None if "emotion_name" in metadata.columns else "neutral"
    items = []
    not_found_counter = 0
    for row in metadata.itertuples():
        if speaker is None and ignored_speakers is not None and row.speaker_name in ignored_speakers:
            continue
        ...
        items.append({
            ...
            "speaker_name": speaker if speaker is not None else row.speaker_name,
            "emotion_name": emotion if emotion is not None else row.emotion_name,
        })
