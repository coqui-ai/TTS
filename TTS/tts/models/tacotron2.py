...
class Tacotron2(TacotronAbstract):
    ...
    def __init__(
        ...
        if num_langs > 1:
            ...
            self.speaker_embedding = nn.Embedding(self.num_speakers, speaker_embedding_dim) # Temporary fix
            self.speaker_embedding.weight.data.normal_(0, 0.3)
        ...
        if self.reversal_classifier:
            self._reversal_classifier = ReversalClassifier(
                input_dim=self.decoder_in_features if not gst else self.decoder_in_features - self.gst_embedding_dim,
