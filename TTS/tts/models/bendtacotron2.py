...
class Bendtacotron2(BaseTacotron):
    ...
    def __init__(self, config: Coqpit, speaker_manager: SpeakerManager = None):
        ...
        # embedding layer
        # char goes in and is represented by a 512-dim vector, the mapping is learned...
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        # B x mel_dim x T_out
        # Add residuals to the raw mel prediction
        postnet_outputs = self.postnet(decoder_outputs)
        postnet_outputs = decoder_outputs + postnet_outputs

        # JCR: wtf is this masking stuff?
        # sequence masking
    ...
    # Here's where inference happens...
    @torch.no_grad()
    def inference(self, text, aux_input=None):
        ...
        # JCR: wtf is aux_input?
        aux_input = self._format_aux_input(aux_input)

        # JCR: could modify embeddings to warp characters, I guess?
        embedded_inputs = self.embedding(text).transpose(1, 2)

        # JCR: encoding from embedding to feature encoding, maybe not super interesting...
        encoder_outputs = self.encoder.inference(embedded_inputs)
        # JCR: But I would be curious to see what happens by modulating the encoding/latent space?

        if self.gst and self.use_gst:
            # B x gst_dim
            encoder_outputs = self.compute_gst(encoder_outputs, aux_input["style_mel"], aux_input["d_vectors"])
            # JCR: Here's where GST can be manipulated...
        ...
        if self.num_speakers > 1:
            ...
            # JCR: Here's where speaker embeddings can be manipulated...
            encoder_outputs = self._concat_speaker_embedding(encoder_outputs, embedded_speakers)

        # JCR: the decoder is a bit of a black box, could make a new one that can be bent more easily..
        #      e.g. this might be fun to modulate...
        decoder_outputs, alignments, stop_tokens = self.decoder.inference(encoder_outputs)

        # Add residuals to the raw MEL decoder_outputs
        postnet_outputs = self.postnet(decoder_outputs)
        postnet_outputs = decoder_outputs + postnet_outputs

        decoder_outputs, postnet_outputs, alignments = self.shape_outputs(decoder_outputs, postnet_outputs, alignments)

        # JCR: ??? of what use are the stop tokens, alignments, etc during inference?
        outputs = {
            "model_outputs": postnet_outputs,
            "decoder_outputs": decoder_outputs,
            "alignments": alignments,
