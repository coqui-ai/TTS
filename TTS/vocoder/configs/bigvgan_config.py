...
@dataclass
class BigVGANConfig(BaseGANVocoderConfig):
    """Defines parameters for FullBand MelGAN vocoder.

    Example:

        >>> from TTS.vocoder.configs import BigVGANConfig
        >>> config = BigVGANConfig()

    Args:
        model (str):
            Model name used for selecting the right model at initialization. Defaults to `bigvgan`.
        discriminator_model (str): One of the discriminators from `TTS.vocoder.models.*_discriminator`. Defaults to
            'bigvgan_discriminator`.
        generator_model (str): One of the generators from TTS.vocoder.models.*`. Every other non-GAN vocoder model is
            considered as a generator too. Defaults to `bigvgan_generator`.
    ...
    discriminator_model: str = "bigvgan_discriminator"
    discriminator_model_params: dict = field(
        default_factory=lambda: {
            "use_spectral_norm": False,
        }
    )
    generator_model: str = "bigvgan_generator"
    # default params to bigger model for (smaller) bigvgan-base params you can check in above Args section
