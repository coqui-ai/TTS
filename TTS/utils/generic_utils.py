...
def check_audio_arguments(audio_config, ap):
    """Check if audio arguments used for computing stats and features are the
    same with the tts/vocoder model config.

    Args:
        audio_config : audio config for stats generated by compute_statistics
            or features generated with compute_features.
        ap : AudioProcessor object with audio settings loaded from the
            config.json.
