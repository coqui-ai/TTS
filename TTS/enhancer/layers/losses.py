...
class BWELoss(torch.nn.Module):
    ...
    def forward(self, y_hat, y, lens):
        ...
        return_dict["l1_mel"] = self.l1_mel(y_hat_mel, y_mel, lens) # Check lens here
        return_dict["l1_spec"] = torch.mean(torch.stack([self.l1_spec[i](y_hat_specs[i], y_specs[i], lens) for i in range(4)]))
