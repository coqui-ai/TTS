...
# %% argparse
parser = argparse.ArgumentParser(description="Compute feature vectors for each wav file in a dataset.")
parser.add_argument(
    "--config_path",
    type=str,
    required=True,
...
parser.add_argument(
    "--data_path",
    type=str,
    help=("Data path for wav files - directory or CSV file. " "If blank taken from config.json"),
)
parser.add_argument("--feature_path", type=str, help=("Path to store features. " "If blank taken from config.json"))
...
feature_paths = [os.path.dirname(wav_file).replace(data_path, feature_path) for wav_file in wav_files]
feature_paths = set(feature_paths)

for path in feature_paths:
    os.makedirs(path, exist_ok=True)

feature_files = [wav_file.replace(data_path, feature_path).replace(".wav", ".npy") for wav_file in wav_files]

# %% calculate and save features
for wav_file, feature_file in tqdm(zip(wav_files, feature_files), total=len(wav_files), ncols=80):
