...
class LSTMSpeakerEncoder(BaseEncoder):
    ...
    def compute_speaker_reference(self, x, num_frames=160, overlap=0.5):
        """
        Generate embeddings for a batch of utterances
        x: 1xTxD
        AÑADIDO POR MI (copiado)
        """
        # num_frames es la sliding window
    ...
    def batch_compute_speaker_references(self, x, seq_lens, num_frames=160, overlap=0.5):
        """
        Generate embeddings for a batch of utterances
        x: BxTxD
        """
        ...
        cur_iter = 0
        print(max_len, num_frames, num_overlap)
        for offset in range(0, max_len, num_frames - num_overlap):
            cur_iter += 1
            end_offset = min(x.shape[1], offset + num_frames)
            frames = x[:, offset:end_offset]
