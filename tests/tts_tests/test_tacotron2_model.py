...
from TTS.tts.configs.shared_configs import GSTConfig, CapacitronVAEConfig
from TTS.tts.configs.tacotron2_config import Tacotron2Config
from TTS.tts.layers.losses import MSELossMasked, TacotronLoss
...
class TacotronCapacitronTrainTest(unittest.TestCase):
    ...
    def test_train_step():
        ...
        config.use_speaker_embedding = True
        config.num_speakers = 10
        config.use_capacitron_vae = True
        config.capacitron_vae = CapacitronVAEConfig()
        ...
        criterion = TacotronLoss(config).to(device)
        criterion_st = nn.BCEWithLogitsLoss().to(device)
        model = Tacotron2(config).to(device)
        model.train()
        model_ref = copy.deepcopy(model)
