...
from TTS.tts.models.vits2 import (
    Vits2,
    Vits2Args,
    Vits2AudioConfig,
    amp_to_db,
...
class TestVits2(unittest.TestCase):
    ...
    def test_init_multispeaker(self):
        num_speakers = 10
        args = Vits2Args(num_speakers=num_speakers, use_speaker_embedding=True)
        model = Vits2(args)
        assertHasAttr(self, model, "emb_g")

        args = Vits2Args(num_speakers=0, use_speaker_embedding=True)
        model = Vits2(args)
        assertHasNotAttr(self, model, "emb_g")

        args = Vits2Args(num_speakers=10, use_speaker_embedding=False)
        model = Vits2(args)
        assertHasNotAttr(self, model, "emb_g")

        args = Vits2Args(d_vector_dim=101, use_d_vector_file=True)
        model = Vits2(args)
        self.assertEqual(model.embedded_speaker_dim, 101)

    def test_init_multilingual(self):
        args = Vits2Args(language_ids_file=None, use_language_embedding=False)
        model = Vits2(args)
        ...
        args = Vits2Args(language_ids_file=LANG_FILE)
        model = Vits2(args)
        self.assertNotEqual(model.language_manager, None)
        self.assertEqual(model.embedded_language_dim, 0)
        assertHasNotAttr(self, model, "emb_l")

        args = Vits2Args(language_ids_file=LANG_FILE, use_language_embedding=True)
        model = Vits2(args)
        self.assertNotEqual(model.language_manager, None)
        self.assertEqual(model.embedded_language_dim, args.embedded_language_dim)
        assertHasAttr(self, model, "emb_l")

        args = Vits2Args(language_ids_file=LANG_FILE, use_language_embedding=True, embedded_language_dim=102)
        model = Vits2(args)
        self.assertNotEqual(model.language_manager, None)
        self.assertEqual(model.embedded_language_dim, args.embedded_language_dim)
        assertHasAttr(self, model, "emb_l")
    ...
    def test_d_vector_forward(self):
        ...
        config = Vits2Config(model_args=args)
    ...
    def test_multilingual_forward(self):
        ...
        args = Vits2Args(language_ids_file=LANG_FILE, use_language_embedding=True, spec_segment_size=10)
        config = Vits2Config(num_speakers=num_speakers, use_speaker_embedding=True, model_args=args)
    ...
    def test_secl_forward(self):
        ...
        model = Vits2(config, speaker_manager=speaker_manager).to(device)
        output_dict = model.forward(
            input_dummy,
            input_lengths,
            spec,
    ...
    def test_train_step(self):
        ...
        with torch.autograd.set_detect_anomaly(True):
            config = Vits2Config(model_args=Vits2Args(num_chars=32, spec_segment_size=10))
            model = Vits2(config).to(device)
            model.train()
            # model to train
            ...
            model_ref = Vits2(config).to(device)
    ...
    def test_train_step_upsampling(self):
        ...
        # setup the model
        with torch.autograd.set_detect_anomaly(True):
            audio_config = Vits2AudioConfig(sample_rate=22050)
            ...
            config = Vits2Config(model_args=model_args, audio=audio_config)
            model = Vits2(config).to(device)
            model.train()
            # model to train
            optimizers = model.get_optimizer()
    ...
    def test_test_run(self):
        config = Vits2Config(model_args=Vits2Args(num_chars=32))
        model = Vits2.init_from_config(config, verbose=False).to(device)
        model.run_data_dep_init = False
        model.eval()
    ...
    def test_load_checkpoint(self):
        ...
        config = Vits2Config(Vits2Args(num_chars=32))
        model = Vits2.init_from_config(config, verbose=False).to(device)
        chkp = {}
    ...
    def test_init_from_config(self):
        ...
        config = Vits2Config(model_args=Vits2Args(num_chars=32, num_speakers=2))
        model = Vits2.init_from_config(config, verbose=False).to(device)
        self.assertTrue(not hasattr(model, "emb_g"))

        config = Vits2Config(model_args=Vits2Args(num_chars=32, num_speakers=2, use_speaker_embedding=True))
        model = Vits2.init_from_config(config, verbose=False).to(device)
